...
def roughness_function(xy_in: numpy.ndarray, z_in: numpy.ndarray, xy_out: numpy.ndarray, search_radius: float,
                 ground_elevations: numpy.ndarray, voxel_heights: numpy.ndarray = numpy.asarray([0, 0.5, 1, 1.5, 2]),
                 leafsize: int = 10, eps: float = 0):
    assert len(xy_in) == len(z_in), f"len(X) of {len(xy_in)} != len(z_in) {len(z_in)}"
    assert type(z_in) is numpy.ndarray and z_in.ndim == 1, f"Expecting z_in to be a 1D numpy array instead got {z_in}"
    assert type(xy_out) is numpy.ndarray and xy_out.ndim == 2, "Expect xy_out to be a 2D numpy array " \
        f" instead got {xy_out}"
...
def main_compare():
    ...
    grid_dem_z = numpy.empty((len(dim_z), len(dim_y), len(dim_x)), dtype=raster_type)
    roughness = xarray.DataArray(grid_dem_z, coords={'z': dim_z, 'y': dim_y, 'x': dim_x}, dims=['z', 'y', 'x'],
    ...
    # Python roughness - function
    start_time = time.time()
    z_out_flat = roughness_function(xy_in, z_in, grid_xy, ground_elevations=z_out_flat, voxel_heights=voxel_heights,
                                    leafsize=leafsize, search_radius=radius, eps=eps)
    z_out = z_out_flat.transpose().reshape(grid_dem_z.shape)
    end_time = time.time()
    rough_time = end_time-start_time
    if(verbose):
        print(f"Roughness function takes {rough_time}")
    roughness.data[0] = z_out
