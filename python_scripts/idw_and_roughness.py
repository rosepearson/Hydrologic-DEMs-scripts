...
def idw_function(
    ...
    assert (
        type(z_in) is numpy.ndarray and z_in.ndim == 1
    ), f"Expecting z_in to be a 1D numpy array instead got {z_in}"
    assert type(xy_out) is numpy.ndarray and xy_out.ndim == 2, (
        "Expect xy_out to be a 2D numpy array " f" instead got {xy_out}"
    )

    tree = scipy.spatial.KDTree(xy_in, leafsize=leafsize)  # build the tree
    tree_index_list = tree.query_ball_point(
        xy_out, r=search_radius, eps=eps
    )  # , eps=0.2)
    ...
    for i, (near_indicies, point) in enumerate(zip(tree_index_list, xy_out)):
        ...
        if len(near_indicies) == 0:  # Set NaN if no values in search region
            ...
            if smoothed_distances.min() == 0:  # incase the of an exact match
                ...
                z_out[i] = (z_in[near_indicies] / (smoothed_distances**power)).sum(
                    axis=0
...
def combined_roughness_function_mewis(
    all_points: numpy.ndarray,
    gnd_points: numpy.ndarray,
    xy_out: numpy.ndarray,
    search_radius: float,
    voxel_heights: numpy.ndarray,
    leafsize: int = 10,
    eps: float = 0,
    smoothing: float = 0,
    power: int = 1,
    ...
    tree_index_list_gnd = tree_gnd.query_ball_point(
        xy_out, r=search_radius, eps=eps
    )  # , eps=0.2)
    ...
    for i, (near_indicies, point) in enumerate(zip(tree_index_list_gnd, xy_out)):
        ...
        if len(near_indicies) == 0:  # Set NaN if no values in search region
            ...
            if smoothed_distances.min() == 0:  # incase the of an exact match
                ...
                z_out_gnd[i] = (
                    z_in_gnd[near_indicies] / (smoothed_distances**power)
                ).sum(axis=0) / (1 / (smoothed_distances**power)).sum(axis=0)
    ...
    for i, near_indicies in enumerate(tree_index_list_all):
        ...
        if len(near_indicies) == 0:  # Set NaN if no values in search region
            ...
            count_below_height = numpy.array(
                [
                    len(near_indicies[near_heights < threshold_elevation])
                    for threshold_elevation in voxel_heights + ground_elevations[i]
                ]
            )
            z_out_all[i] = numpy.log(
                count_below_height[1:] / count_below_height[:-1]
            ) / (voxel_heights[1:] - voxel_heights[:-1])
    return z_out_gnd, z_out_all


def roughness_function_mewis(
    xy_in: numpy.ndarray,
    z_in: numpy.ndarray,
    xy_out: numpy.ndarray,
    search_radius: float,
    ground_elevations: numpy.ndarray,
    ground_std: numpy.ndarray,
    voxel_heights: numpy.ndarray,
    leafsize: int = 10,
    eps: float = 0,
):
...
def mewis_n_function(
    veg_density: xarray.DataArray,
    depth: xarray.DataArray,
    voxel_total_heights: numpy.ndarray,
    minimum_height: float = 20,
    drag_coefficient: float = 1.2,
):
    assert numpy.all(
        veg_density.x == depth.x
    ), "Expect the same x grids for veg_density and depth."
    assert numpy.all(
        veg_density.y == depth.y
    ), "Expect the same y grids for veg_density and depth."
    ...
    n_coefficient = n_coefficient.assign_attrs(
        {"long_name": "The Manning's n coefficient"}
    )
    n_coefficient.name = "n"

    # Calculate lambda (The Darcy-Weisbach friction coefficient) using the Mewis paper equation
    for i in range(len(veg_density.z)):
        n_coefficient.data[depth.data > voxel_total_heights[i + 1]] += (
            4
            * drag_coefficient
            * veg_density.data[i, depth.data > voxel_total_heights[i + 1]]
            * (voxel_total_heights[i + 1] - voxel_total_heights[i])
        )
        n_coefficient.data[
            (depth.data > voxel_total_heights[i])
            & (depth.data < voxel_total_heights[i + 1])
        ] += (
            4
            * drag_coefficient
            * veg_density.data[
            ...
            * (
                ...
                - voxel_total_heights[i]
            )
        )
    # Convert to Manning's n
    n_coefficient.data = numpy.sqrt(
        n_coefficient.data * numpy.cbrt(depth.data) / (8 * 9.8)
    )

    return n_coefficient


def roughness_function_graham(
    xy_in: numpy.ndarray,
    z_in: numpy.ndarray,
    xy_out: numpy.ndarray,
    search_radius: float,
    ground_elevations: numpy.ndarray,
    voxel_heights: numpy.ndarray = numpy.asarray([0, 0.5, 1, 1.5, 2]),
    leafsize: int = 10,
    eps: float = 0,
):
    assert len(xy_in) == len(z_in), f"len(X) of {len(xy_in)} != len(z_in) {len(z_in)}"
    ...
    assert (
        voxel_heights[-1] > voxel_heights[0]
    ), "The voxel heights must be monotonically increasing"
...
def setup_pdal(h_crs, v_crs, verbose=True):
    ...
    pdal_pipeline_instructions = [
        {"type": "readers.las", "filename": str(tile_name)},
        {
            "type": "filters.reprojection",
            "in_srs": f"EPSG:{h_crs}+{v_crs}",
...
def main_compare():
    ...
    grid_x, grid_y = numpy.meshgrid(
        numpy.arange(xy_in[:, 0].min(), xy_in[:, 0].max(), res),
        numpy.arange(xy_in[:, 1].min(), xy_in[:, 1].max(), res),
    )
    grid_xy = numpy.concatenate(
        (grid_x.flatten().reshape((1, -1)), grid_y.flatten().reshape((1, -1))), axis=0
    ).transpose()
    ...
    z_out_flat, z_std_flat = idw_function(
        xy_in,
        z_in,
        grid_xy,
        leafsize=leafsize,
    ...
    voxel_total_heights = numpy.concatenate(
        [dim_z - voxel_cell_heights / 2, [dim_z[-1] + voxel_cell_heights[-1] / 2]]
    )  # bottom to top of each voxel
    print(
        f"Voxel bin middles is {dim_z}, cell_heights are {voxel_cell_heights}, and voxel bottoms and tops are {voxel_total_heights}"
    )
    grid_dem_r = numpy.empty((len(dim_z), len(dim_y), len(dim_x)), dtype=raster_type)
    veg_density = xarray.DataArray(
        grid_dem_r,
        coords={"z": dim_z, "y": dim_y, "x": dim_x, "h": ("z", voxel_cell_heights)},
        dims=["z", "y", "x"],
        attrs={
            "scale_factor": 1.0,
            "add_offset": 0.0,
            "long_name": "vegetation density",
        },
    )
    ...
    z_gnd, z_all = combined_roughness_function_mewis(
        all_points,
        gnd_points,
        grid_xy,
        voxel_heights=voxel_total_heights,
        leafsize=leafsize,
        search_radius=radius,
        power=power,
        eps=eps,
    ...
    start_time = time.time()
    rough_flat = roughness_function_mewis(
        xy_in,
        z_in,
        grid_xy,
        ground_elevations=z_out_flat,
        ground_std=z_std_flat,
        voxel_heights=voxel_total_heights,
        leafsize=leafsize,
        search_radius=radius,
        eps=eps,
    ...
    for i in range(len(dim_z)):
        fig, ax = matplotlib.pyplot.subplots()
        veg_density.isel(z=i).plot(
            vmin=vmin, vmax=vmax, norm=matplotlib.colors.LogNorm()
        )
        ax.set_title(
            f"Log plot: vegetation density bin {voxel_total_heights[i]}-{voxel_total_heights[i+1]}m"
        )
    """
...
def main_manual_roughness(dem: xarray.DataArray):
    ...
    base_path = pathlib.Path(
        r"C:\Users\pearsonra\Documents\data\roughness\Waikanae_roughness"
    )
    ...
    zo_array = xarray.DataArray(
        zo_clipped,
        coords={"y": dim_y[dim_y_mask], "x": dim_x[dim_x_mask]},
        dims=["y", "x"],
        attrs={
            "scale_factor": 1.0,
            "add_offset": 0.0,
            "long_name": "zo manual from Graeme",
        },
    )
...
def main_calculate_mannings_n(veg_density: xarray.DataArray):
    ...
    dim_x_mask = (dim_x > float(veg_density.x.min())) & (
        dim_x < float(veg_density.x.max())
    )
    dim_y_mask = (dim_y > float(veg_density.y.min())) & (
        dim_y < float(veg_density.y.max())
    )

    depth_clipped = depth[dim_y_mask, :]
    depth_clipped = depth_clipped[:, dim_x_mask]

    depth_clipped = xarray.DataArray(
        depth_clipped,
        coords={"y": dim_y[dim_y_mask], "x": dim_x[dim_x_mask]},
        dims=["y", "x"],
        attrs={
    ...
    depth_aligned_with_min.data[
        numpy.isnan(depth_aligned_with_min.data)
    ] = minimum_height
    ...
    mannings_n_mewis = mewis_n_function(
        veg_density,
        depth_aligned_with_min,
        voxel_total_heights,
